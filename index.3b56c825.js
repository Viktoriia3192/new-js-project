...
const{Axios:tY,AxiosError:tJ,CanceledError:tX,isCancel:tQ,CancelToken:tZ,VERSION:t0,all:t1,Cancel:t2,isAxiosError:t4,spread:t9,toFormData:t5,AxiosHeaders:t6,HttpStatusCode:t3,formToJSON:t8,getAdapter:t7,mergeConfig:ne}=tG;async function nt(e){try{let t=await tG.get("https://books-backend.p.goit.global/books/"+e);return t.data}catch(e){console.error("An error occurred while fetching data:",e)}}async function nn(){let{data:e}=await tG.get("https://books-backend.p.goit.global/books/category-list");return e}const ni=document.querySelector(".categories-list");(async function e(){try{let e=await nn();if(!e.length){(0,D.Notify).failure("Oops something going wrong.");return}ni.insertAdjacentHTML("beforeend",e.map(({list_name:e})=>`
  ...
  `).join(""))}catch(e){console.log("TRY-CATCH:",e),(0,D.Notify).failure("Oops something going wrong.")}})(),ni.addEventListener("click",e=>{e.preventDefault()}),P("fewGv");var nr=P("7Y9D8");const ns=(e,t)=>{try{let n=JSON.stringify(t);localStorage.setItem(e,n)}catch(e){(0,nr.Notify).error("Set state error: ",e.message)}},no=e=>{try{let t=localStorage.getItem(e);return null===t?void 0:JSON.parse(t)}catch(e){(0,nr.Notify).error("Get state error: ",e.message)}};var nr=P("7Y9D8");!function(){let e=document.querySelector(".loader");e.style.display="block"}();const na=document.querySelector(".common-list"),nl=document.querySelector(".main-title"),nh=document.querySelector(".categories-list");let nc=window.innerWidth<767?1:window.innerWidth<1439?3:5;function nu(e){let t=function(e){let t=[...e];for(let e=t.length-1;e>0;e--){let n=Math.floor(Math.random()*(e+1));[t[e],t[n]]=[t[n],t[e]]}return t}(e);!function(){let e=document.querySelector(".loader");e.style.display="none"}();let n="";for(let e of(nl.textContent="Best Sellers",nl.insertAdjacentHTML("beforeend",'&nbsp;<span class="main-title main-title-wrapper">Books</span>'),t)){let{books:t,list_name:i}=e;n+=` 
        <li class="common-item">
        <h2 class="common-title">${i}</h2>
        <ul class="book-list">
            ${function(e,t){let n="";for(let i=0;i<t;i+=1){let{book_image:t,title:r,author:s,_id:o}=e[i];n+=`
    ...
    </li>`}na.innerHTML=n}nh.firstElementChild.addEventListener("click",function(){let e=no("data");nu(e)}),window.addEventListener("resize",function(){let e=window.innerWidth<767?1:window.innerWidth<1439?3:5;if(e!=nc){nc=e;let t=no("data");nu(t)}}),nt("top-books").then(e=>{ns("data",e),nu(e)}).catch(e=>(0,nr.Notify).failure(e));var nr=P("7Y9D8");const nd={categories:document.querySelector(".categories-list"),title:document.querySelector(".main-title"),list:document.querySelector(".common-list")},nf="Sorry, there are no books matching the selected category. Please select something else.",np="Sorry, something went wrong. Try again!";nd.categories.addEventListener("click",function(e){"categories-link"===e.target.className&&(nI(e.target),ng=e.target.textContent.replaceAll(" ","%20"),e.target!==nd.categories.firstElementChild.firstElementChild&&n_(ng).then(e=>{if(0===e.data.length)throw Error((0,nr.Notify).info(nf));let t=e.data[0].list_name;nd.title.textContent=nw(t),nd.title.insertAdjacentHTML("beforeend",`&nbsp;<span class="main-title main-title-wrapper">${nb(t)}</span>`),nd.list.innerHTML=nv(e.data)}).catch(function(e){e.response?(0,nr.Notify).failure(np):e.request&&(0,nr.Notify).failure(np)}))}),nd.list.addEventListener("click",function(e){if("showMore-btn"!==e.target.className)return;nm=e.target.name.replaceAll(" ","%20");let t=Array.from(nd.categories.children);t.forEach(e=>{let t=e.firstElementChild.textContent.replaceAll(" ","%20");t===nm&&nI(e)}),n_(nm).then(e=>{if(0===e.data.length)throw Error((0,nr.Notify).info(nf));let t=e.data[0].list_name;nd.title.textContent=nw(t),nd.title.insertAdjacentHTML("beforeend",`&nbsp;<span class="main-title main-title-wrapper">${nb(t)}</span>`),nd.list.innerHTML=nv(e.data)}).catch(function(e){e.response?(0,nr.Notify).failure(np):e.request&&(0,nr.Notify).failure(np)})});let ng="",nm="",ny=nd.categories.firstElementChild.firstElementChild;async function n_(e){let t=`https://books-backend.p.goit.global/books/category?category=${e}`,n=await tG.get(t);return n}function nv(e){return e.map(({book_image:e,author:t,title:n,_id:i})=>{let r=`<li class="book-item" data-id="${i}" tabindex="0">
...
nS.innerHTML="",nS.insertAdjacentHTML("afterbegin",function({author:e,title:t,description:n,book_image:i,buy_links:r}){let s=`<div class="modal__img-container"> 
      ...
      <div class="modal__desc"> 
        ...
        <li>
        <a class="modal-book-shop" href=${nM(r,"Amazon")} target= _blank><img src="${/*@__PURE__*/A(nE)}" alt="Amazon"></a>
...
var g$=document.querySelector(".auth-button-in"),gV=document.querySelector(".auth-input");g$.addEventListener("click",function(){gV&&gV.remove()}),P("dGLOO");//# sourceMappingURL=index.3b56c825.js.map

//# sourceMappingURL=index.3b56c825.js.map
