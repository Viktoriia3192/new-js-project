...
let{Axios:tJ,AxiosError:tY,CanceledError:tX,isCancel:tQ,CancelToken:tZ,VERSION:t0,all:t1,Cancel:t2,isAxiosError:t4,spread:t9,toFormData:t5,AxiosHeaders:t6,HttpStatusCode:t3,formToJSON:t8,getAdapter:t7,mergeConfig:ie}=tG;async function it(e){try{let t=await tG.get("https://books-backend.p.goit.global/books/"+e);return t.data}catch(e){console.error("An error occurred while fetching data:",e)}}async function ii(){let{data:e}=await tG.get("https://books-backend.p.goit.global/books/category-list");return e}let ir=document.querySelector(".categories-list");(async function e(){try{let e=await ii();if(!e.length){(0,D.Notify).failure("Oops something going wrong.");return}ir.insertAdjacentHTML("beforeend",e.map(({list_name:e})=>`
  ...
  `).join(""))}catch(e){console.log("TRY-CATCH:",e),(0,D.Notify).failure("Oops something going wrong.")}})(),ir.addEventListener("click",e=>{e.preventDefault()}),P("ksGdE");var is=P("6JpON");let io=(e,t)=>{try{let i=JSON.stringify(t);localStorage.setItem(e,i)}catch(e){(0,is.Notify).error("Set state error: ",e.message)}},ia=e=>{try{let t=localStorage.getItem(e);return null===t?void 0:JSON.parse(t)}catch(e){(0,is.Notify).error("Get state error: ",e.message)}};var is=P("6JpON");!function(){let e=document.querySelector(".loader");e.style.display="block"}();let il=document.querySelector(".common-list"),ih=document.querySelector(".main-title"),iu=document.querySelector(".categories-list"),ic=window.innerWidth<767?1:window.innerWidth<1439?3:5;function id(e){let t=function(e){let t=[...e];for(let e=t.length-1;e>0;e--){let i=Math.floor(Math.random()*(e+1));[t[e],t[i]]=[t[i],t[e]]}return t}(e);!function(){let e=document.querySelector(".loader");e.style.display="none"}();let i="";for(let e of(ih.textContent="Best Sellers",ih.insertAdjacentHTML("beforeend",'&nbsp;<span class="main-title main-title-wrapper">Books</span>'),t)){let{books:t,list_name:n}=e;i+=` 
        <li class="common-item">
        <h2 class="common-title">${n}</h2>
        <ul class="book-list">
            ${function(e,t){let i="";for(let n=0;n<t;n+=1){let{book_image:t,title:r,author:s,_id:o}=e[n];i+=`
    ...
    </li>`}il.innerHTML=i}iu.firstElementChild.addEventListener("click",function(){let e=ia("data");id(e)}),window.addEventListener("resize",function(){let e=window.innerWidth<767?1:window.innerWidth<1439?3:5;if(e!=ic){ic=e;let t=ia("data");id(t)}}),it("top-books").then(e=>{io("data",e),id(e)}).catch(e=>(0,is.Notify).failure(e));var is=P("6JpON");let ip={categories:document.querySelector(".categories-list"),title:document.querySelector(".main-title"),list:document.querySelector(".common-list")},ig="Sorry, there are no books matching the selected category. Please select something else.",im="Sorry, something went wrong. Try again!";ip.categories.addEventListener("click",function(e){"categories-link"===e.target.className&&(iT(e.target),iy=e.target.textContent.replaceAll(" ","%20"),e.target!==ip.categories.firstElementChild.firstElementChild&&iw(iy).then(e=>{if(0===e.data.length)throw Error((0,is.Notify).info(ig));let t=e.data[0].list_name;ip.title.textContent=iI(t),ip.title.insertAdjacentHTML("beforeend",`&nbsp;<span class="main-title main-title-wrapper">${iE(t)}</span>`),ip.list.innerHTML=ib(e.data)}).catch(function(e){e.response?(0,is.Notify).failure(im):e.request&&(0,is.Notify).failure(im)}))}),ip.list.addEventListener("click",function(e){if("showMore-btn"!==e.target.className)return;i_=e.target.name.replaceAll(" ","%20");let t=Array.from(ip.categories.children);t.forEach(e=>{let t=e.firstElementChild.textContent.replaceAll(" ","%20");t===i_&&iT(e)}),iw(i_).then(e=>{if(0===e.data.length)throw Error((0,is.Notify).info(ig));let t=e.data[0].list_name;ip.title.textContent=iI(t),ip.title.insertAdjacentHTML("beforeend",`&nbsp;<span class="main-title main-title-wrapper">${iE(t)}</span>`),ip.list.innerHTML=ib(e.data)}).catch(function(e){e.response?(0,is.Notify).failure(im):e.request&&(0,is.Notify).failure(im)})});let iy="",i_="",iv=ip.categories.firstElementChild.firstElementChild;async function iw(e){let t=`https://books-backend.p.goit.global/books/category?category=${e}`,i=await tG.get(t);return i}function ib(e){return e.map(({book_image:e,author:t,title:i,_id:n})=>{let r=`<li class="book-item" data-id="${n}" tabindex="0">
...
iR.innerHTML="",iR.insertAdjacentHTML("afterbegin",function({author:e,title:t,description:i,book_image:n,buy_links:r}){let s=`<div class="modal__img-container"> 
      ...
      <div class="modal__desc"> 
        ...
        <li>
        <a class="modal-book-shop" href=${iF(r,"Amazon")} target= _blank><img src="${/*@__PURE__*/A(iC)}" alt="Amazon"></a>
...
var g$=document.querySelector(".auth-button-in"),gV=document.querySelector(".auth-input");g$.addEventListener("click",function(){gV&&gV.remove()}),P("E6hPi")}();//# sourceMappingURL=index.a5e86c02.js.map

//# sourceMappingURL=index.a5e86c02.js.map
