...
var N,C,I=function(){k.classList.add("nx-remove");var e=t.document.getElementById(i.overlayID);e&&x.childElementCount<=0&&e.classList.add("nx-remove"),clearTimeout(N)},z=function(){if(k&&null!==k.parentNode&&k.parentNode.removeChild(k),x.childElementCount<=0&&null!==x.parentNode){x.parentNode.removeChild(x);var e=t.document.getElementById(i.overlayID);e&&null!==e.parentNode&&e.parentNode.removeChild(e)}clearTimeout(C)};// if callbackOrOptions or click to close: end
// else auto remove: begin
if(e.closeButton&&"function"!=typeof c&&t.document.getElementById(b.id).querySelector("span.nx-close-button").addEventListener("click",function(){I();var t=setTimeout(function(){z(),clearTimeout(t)},e.cssAnimationDuration)}),("function"==typeof c||e.clickToClose)&&k.addEventListener("click",function(){"function"==typeof c&&c(),I();var t=setTimeout(function(){z(),clearTimeout(t)},e.cssAnimationDuration)}),!e.closeButton&&"function"!=typeof c){// auto remove: begin
var A=function(){N=setTimeout(function(){I()},e.timeout),C=setTimeout(function(){z()},e.timeout+e.cssAnimationDuration)};A(),e.pauseOnHover&&(k.addEventListener("mouseenter",function(){k.classList.add("nx-paused"),clearTimeout(N),clearTimeout(C)}),k.addEventListener("mouseleave",function(){k.classList.remove("nx-paused"),A()}));// pause auto remove: end
}// else auto remove: end
}// remove by timeout or click: end
// notify - show only the last one: begin
if(e.showOnlyTheLastOne&&f>0)for(var W=t.document.querySelectorAll("[id^="+e.ID+"-]:not([id="+e.ID+"-"+f+"])"),L=0;L<W.length;L++){var E=W[L];null!==E.parentNode&&E.parentNode.removeChild(E)}// notify - show only the last one: end
...
info:function(t,e,i){m(o.Info,t,e,i)}}};return"object"==typeof t.Notiflix?s(!0,t.Notiflix,{Notify:d.Notify}):{Notify:d.Notify}},"function"==typeof define&&define.amd?define([],function(){return e(t)}):"object"==typeof s?s=e(t):t.Notiflix=e(t);var l=a("2shzp");async function c(t){try{let e=await (0,l.default).get("https://books-backend.p.goit.global/books/"+t);return e.data}catch(t){console.error("An error occurred while fetching data:",t)}}async function f(){let{data:t}=await (0,l.default).get("https://books-backend.p.goit.global/books/category-list");return t}const m=document.querySelector(".categories-list");(async function t(){try{let t=await f();if(!t.length){(0,s.Notify).failure("Oops something going wrong.");return}m.insertAdjacentHTML("beforeend",t.map(({list_name:t})=>`
    <li class="categories-list-item">
      <a href="#" class="categories-link">${t}</a>
    </li>
  `).join(""))}catch(t){console.log("TRY-CATCH:",t),(0,s.Notify).failure("Oops something going wrong.")}})(),m.addEventListener("click",t=>{t.preventDefault()}),a("fewGv");const d=(t,e)=>{try{let o=JSON.stringify(e);localStorage.setItem(t,o)}catch(t){console.error("Set state error: ",t.message)}},p=t=>{try{let e=localStorage.getItem(t);return null===e?void 0:JSON.parse(e)}catch(t){console.error("Get state error: ",t.message)}};!function(){let t=document.querySelector(".loader");t.style.display="block"}();const y=document.querySelector(".common-list");document.querySelector(".main-title");const u=document.querySelector(".categories-list");let x=window.innerWidth<767?1:window.innerWidth<1439?3:5;function g(t){!function(){let t=document.querySelector(".loader");t.style.display="none"}();let e="";for(let o of t){let{books:t,list_name:i}=o;e+=` 
    ...
    </li>`}y.innerHTML=e}u.firstElementChild.addEventListener("click",function(){let t=p("data");console.log(t),g(t)}),window.addEventListener("resize",function(){let t=window.innerWidth<767?1:window.innerWidth<1439?3:5;if(t!=x){x=t;let e=p("data");g(e)}}),c("top-books").then(t=>{d("data",t),g(t)}).catch(t=>console.log(t));var l=a("2shzp"),b=a("7Y9D8");const h={categories:document.querySelector(".categories-list"),title:document.querySelector(".main-title"),list:document.querySelector(".common-list")},w="Sorry, there are no books matching the selected category. Please select something else.",v="Sorry, something went wrong. Try again!";h.categories.addEventListener("click",function(t){"categories-link"===t.target.className&&(W(t.target),k=t.target.textContent.replaceAll(" ","%20"),t.target!==h.categories.firstElementChild.firstElementChild&&C(k).then(t=>{if(0===t.data.length)throw Error((0,b.Notify).info(w));let e=t.data[0].list_name;h.title.textContent=z(e),h.title.insertAdjacentHTML("beforeend",`&nbsp;<span class="main-title main-title-wrapper">${A(e)}</span>`),h.list.innerHTML=I(t.data)}).catch(function(t){t.response?(0,b.Notify).failure(v):t.request&&(0,b.Notify).failure(v)}))}),h.list.addEventListener("click",function(t){"showMore-btn"===t.target.className&&(console.log(t.target),C(t.target.name.replaceAll(" ","%20")).then(t=>{if(0===t.data.length)throw Error((0,b.Notify).info(w));let e=t.data[0].list_name;h.title.textContent=z(e),h.title.insertAdjacentHTML("beforeend",`&nbsp;<span class="main-title main-title-wrapper">${A(e)}</span>`),h.list.innerHTML=I(t.data)}).catch(function(t){t.response?(0,b.Notify).failure(v):t.request&&(0,b.Notify).failure(v)}))});let k="",N=h.categories.firstElementChild.firstElementChild;async function C(t){let e=`https://books-backend.p.goit.global/books/category?category=${t}`,o=await (0,l.default).get(e);return o}function I(t){return t.map(({book_image:t,author:e,title:o,_id:i})=>{let n=`<li class="book-item" data-id="${i}">
            <a href="#" class="book-link">
                <img class="book-img" src="${t||"../images/default_image.jpg"}" data-id="${i}" alt="${o}"> 
                <h3 class="book-title">${o}</h3>
                <p class="book-author">${e}</p>
            </a>
        </li>`;return n}).join("")}function z(t){let e=t.split(" ");return e.splice(0,e.length-1).join(" ")}function A(t){let e=t.split(" ");return e[e.length-1]}function W(t){N&&N.classList.remove("category-active"),t.classList.add("category-active"),N=t}W(N),a("aZ25t"),// import { initializeApp } from "firebase/app";
...
//     }
// });
a("dGLOO");//# sourceMappingURL=index.efdc64de.js.map

//# sourceMappingURL=index.efdc64de.js.map
