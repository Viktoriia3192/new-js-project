...
function checkResize() {
  // Check wether we should render markup or not.

  let currentLength =
    window.innerWidth < 767 ? 1 : window.innerWidth < 1439 ? 3 : 5;
...
function renderMarkup(data) {
  ...
  title.insertAdjacentHTML(
    'beforeend',
    `&nbsp;<span class="main-title main-title-wrapper">${secondTitlePart}</span>`
  );
  ...
  function booksRender(books, length) {
    // render books inside the category
